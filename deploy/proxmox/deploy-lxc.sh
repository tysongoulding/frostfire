#!/usr/bin/env bash
# ==============================================================================
# Frostfire Cloud Proxmox VE Automated LXC Deployment Script
# Run directly on Proxmox VE Host Shell (pve-shell)
# ==============================================================================
set -euo pipefail

CT_ID="${1:-$(pvesh get /cluster/nextid)}"
CT_HOSTNAME="${2:-frostfire-cloud}"
STORAGE="${3:-local-lvm}"
BRIDGE="${4:-vmbr0}"
MEMORY="${5:-2048}"
CORES="${6:-2}"
...
echo "=========================================================="
echo "🚀 Deploying Frostfire Cloud on Proxmox VE"
...
pct exec "${CT_ID}" -- bash -c "
    ...
    # Deploy Frostfire Cloud services
    mkdir -p /opt/frostfire
    cat << 'EOF' > /opt/frostfire/docker-compose.yml
services:
  gateway:
    image: ghcr.io/tysongoulding/frostfire-cloud:latest
    container_name: frostfire-gateway
    restart: always
    ports:
      - "50051:50051"
    command: ["frostfire-gateway", "--bind", "0.0.0.0:50051"]
    environment:
      - RUST_LOG=info
  orchestrator:
    image: ghcr.io/tysongoulding/frostfire-cloud:latest
    container_name: frostfire-orchestrator
    restart: always
    command: ["frostfire-orchestrator", "--sprint-id", "sprint-default", "--objective", "Continuous Swarm"]
    environment:
      - RUST_LOG=info
    depends_on:
      - gateway
EOF
    ...
    cat << 'EOF' > /etc/systemd/system/frostfire-cloud.service
[Unit]
Description=Frostfire Cloud Docker Compose Stack
...
Type=oneshot
RemainAfterExit=yes
WorkingDirectory=/opt/frostfire
...
EOF
    ...
    systemctl enable frostfire-cloud
    systemctl start frostfire-cloud || true
"
...
echo "=========================================================="
echo "✅ Frostfire Cloud LXC Container Deployed Successfully!"
