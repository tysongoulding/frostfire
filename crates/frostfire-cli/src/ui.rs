...
async fn handle_connection(
    ...
    match (method, path) {
        ...
        ("GET", "/api/audit") => {
            ...
            let audit_path = workspace.join(".frostfire").join("audit.db");
...
async fn execute_turn_via_tunnel(
    ...
    let audit_dir = workspace.join(".frostfire");
        ...
        Err(_) => {
            ...
            for frame in plan.server_frames_to_send {
                if let Some(p) = frame.payload {
                    match p {
                        frostfire_proto::tunnel::tunnel_server_frame::Payload::AgentMessage(_) => {}
...
async fn execute_tool_frame_locally(
    payload: frostfire_proto::tunnel::tunnel_server_frame::Payload,
    jail: &WorkspaceJail,
...
mod tests {
    ...
    async fn test_ui_http_server_endpoints() {
        ...
        assert!(resp.contains("Frostfire Swarm"));
