...
mod tests {
    ...
    async fn test_pty_screen_lifecycle() {
        ...
        #[cfg(windows)]
        let opts = SpawnOptions::new("cmd.exe").args(["/c", "echo hello_frostfire"]);
        #[cfg(not(windows))]
        let opts = SpawnOptions::new("echo").arg("hello_frostfire");
        ...
        assert!(output_str.contains("hello_frostfire"));
        ...
        assert!(String::from_utf8_lossy(&history).contains("hello_frostfire"));
