...
//!
//! Allocates isolated worktree environments under `.frostfire/worktrees/<agent_id>`
...
impl WorktreeManager {
    ...
    pub fn new(repo_root: impl AsRef<Path>) -> Result<Self, WorktreeError> {
        ...
        let worktrees_base = repo_root.join(".frostfire").join("worktrees");
    ...
    ///
    /// The worktree is created at `.frostfire/worktrees/<agent_id>` with branch
    /// `frostfire/worktree-<agent_id>`.
    pub fn create_worktree(
        ...
        let branch_name = format!("frostfire/worktree-{}", agent_id);
    ...
    /// Lists active worktrees managed under `.frostfire/worktrees`.
    pub fn list_worktrees(&self) -> Result<Vec<WorktreeInfo>, WorktreeError> {
        ...
        for entry in entries.flatten() {
            if entry.path().is_dir() {
                ...
                let branch = format!("frostfire/worktree-{}", name);
    ...
    /// Cleans up all managed worktrees under `.frostfire/worktrees`.
    ...
    fn test_worktree_lifecycle_with_git() {
        ...
        if let Ok(out) = init {
            if out.status.success() {
                ...
                let _ = Command::new("git")
                    .current_dir(&temp_dir)
                    .args(["config", "user.name", "Frostfire Test"])
                    .output();
                let _ = Command::new("git")
                    .current_dir(&temp_dir)
                    .args(["config", "user.email", "test@frostfire.dev"])
                    .output();

                // Initial commit
                let file_path = temp_dir.join("README.md");
                std::fs::write(&file_path, "# Frostfire Git Test").unwrap();
                let _ = Command::new("git")
                    .current_dir(&temp_dir)
                    .args(["add", "."])
                    .output();
