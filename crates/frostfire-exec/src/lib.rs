//! `frostfire-exec`
//!
//! Sandboxed execution engine for Frostfire agents.
...
"#;
    ...
    async fn test_pty_screen_io_lifecycle() {
        ...
        #[cfg(windows)]
        let opts = SpawnOptions::new("cmd.exe").args(["/c", "echo hello_frostfire_pty"]);
        #[cfg(not(windows))]
        let opts = SpawnOptions::new("echo").arg("hello_frostfire_pty");
        ...
        assert!(output.contains("hello_frostfire_pty"));

        let history = mux.get_history(&screen_id).unwrap();
        assert!(String::from_utf8_lossy(&history).contains("hello_frostfire_pty"));
