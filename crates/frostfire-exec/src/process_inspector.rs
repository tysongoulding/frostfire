...
impl ProcessInspector {
    ...
    #[cfg(target_os = "linux")]
    pub fn list_linux_processes(
        proc_root: impl AsRef<Path>,
    ) -> Result<Vec<ProcessEntry>, InspectorError> {
        ...
        for entry in dir.flatten() {
            ...
            if let Ok(pid) = file_name.parse::<u32>() {
                ...
                let comm = std::fs::read_to_string(path.join("comm"))
                    ...
                    .trim()
                    .to_string();
                let cmdline = std::fs::read(path.join("cmdline")).ok().map(|bytes| {
                    bytes
                        .split(|&b| b == 0)
                        .filter(|s| !s.is_empty())
    ...
    fn list_windows_processes() -> Result<Vec<ProcessEntry>, InspectorError> {
        ...
        unsafe {
            ...
            if snapshot == (-1isize as *mut std::ffi::c_void) || snapshot.is_null() {
                return Err(InspectorError::Platform(
...
mod tests {
    ...
    fn test_process_inspector_queries_current_process() {
        ...
        assert!(
            current.is_some(),
            "Current process should be in the process table"
        );
