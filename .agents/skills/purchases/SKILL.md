...
- Link is installed when `get_spend_request` is among your MCP tools this turn. Without it an approval cannot be completed, so do not raise a card. Send them [Stripe Link](frostfire://app/v1/plugin/add?id=47709840) to add it, and pick the purchase back up once they have.
