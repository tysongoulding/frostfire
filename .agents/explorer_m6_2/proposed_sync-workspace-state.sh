...
# ------------------------------------------------------------------------------
PERSIST_ROOT="${FROSTFIRE_PERSIST_ROOT:-/mnt/workspace}"
DEFAULT_WORKSPACE="${WORKSPACE_DIR:-/mnt/workspace}"
...
do_snapshot() {
    ...
    if is_git_repo "$ws_dir"; then
        ...
        git -C "$ws_dir" ls-files --others --exclude-standard -z 2>/dev/null | \
            grep -zv "^\.frostfire" > "$untracked_list" || true
        ...
        if [ -s "$untracked_list" ]; then
            ...
            is_dirty="true"
            # Update latest untracked symlink/pointer
...
"
    ...
    if [ "$is_git" = "True" ] || [ "$is_git" = "true" ]; then
        ...
        if [ -n "$untracked_file" ] && [ -f "${state_dir}/${untracked_file}" ]; then
            local actual_sha256
            actual_sha256="$(sha256sum "${state_dir}/${untracked_file}" | cut -d' ' -f1)"
            ...
            vlog "Extracting untracked files from ${untracked_file}"
            tar -C "$ws_dir" -xzf "${state_dir}/${untracked_file}" 2>/dev/null
        fi
        ...
        if [ -n "$archive_file" ] && [ -f "${state_dir}/${archive_file}" ]; then
            ...
            actual_archive_sha="$(sha256sum "${state_dir}/${archive_file}" | cut -d' ' -f1)"
            ...
            vlog "Restoring non-git workspace from ${archive_file}"
            tar -C "$ws_dir" -xzf "${state_dir}/${archive_file}" 2>/dev/null
        else
