...
impl GeminiClient {
    ...
    pub fn resolve_api_key() -> Option<String> {
        ...
        // Check .frostfire/gemini.key in current working directory
        let local_path = std::path::Path::new(".frostfire").join("gemini.key");
    ...
    pub fn from_env() -> Self {
        ...
        match Self::resolve_api_key() {
            ...
            None => {
                info!("GeminiClient: no API key found in env or ~/.config/frostfire/gemini.key, initializing in offline Dev Mock mode");
                Self::dev_mock()
            }
        }
    }
