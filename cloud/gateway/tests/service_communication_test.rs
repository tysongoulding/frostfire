...
async fn test_cloud_service_communicates_with_application_crates() {
    // 1. Initialize Cloud Track Service (frostfire-gateway) on ephemeral port
    let gateway = Arc::new(
    ...
    // 2. Configure Application Track Crate (frostfire-daemon) to point at Cloud Gateway
    let temp_dir = std::env::temp_dir().join(format!("frostfire_test_{}", uuid::Uuid::new_v4()));
    ...
    let patch_frame = TunnelServerFrame {
        ...
        payload: Some(tunnel_server_frame::Payload::ApplyPatch(
            frostfire_proto::tunnel::ApplyPatch {
...
async fn test_cloud_gateway_evaluates_user_prompt_and_returns_tool_frames() {
    ...
    let gemini = Arc::new(frostfire_orchestrator::GeminiClient::dev_mock());
    let engine = Arc::new(frostfire_orchestrator::AgentTurnEngine::new(gemini));
    let gateway = Arc::new(
    ...
    let prompt_frame = frostfire_proto::tunnel::TunnelClientFrame {
        ...
        timestamp_unix_ms: chrono::Utc::now().timestamp_millis(),
        payload: Some(frostfire_proto::tunnel::tunnel_client_frame::Payload::UserPrompt(
            frostfire_proto::tunnel::UserPrompt {
